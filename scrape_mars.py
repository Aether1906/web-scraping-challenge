...
def scrape_all():
   ...
   # the goal is to return a json that has all of the nessessary data so that it can be loaded into mongoDB

   # get the information from the news page
    news_title, news_paragraph = scrape_news(browser)

    #build the dictionary using the info from the scrapes
    marsData = {
...
#scrape mars new page
def scrape_news(browser):
    # got to the Mar Nasa news site
...
def scrape_facts_page(browser):
    ...
    #add the text to an empty string then return
    facts += str(factTable)
